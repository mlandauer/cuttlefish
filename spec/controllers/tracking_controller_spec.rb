...
describe TrackingController, type: :controller do
  describe "#open" do
    ...
    it "should be succesful when the correct hash is used" do
      ...
      get :open, params: { delivery_id: 101, hash: "e4a8656f793ded530fb9d619af3c6c08a49ead7f" }
      expect(response).to be_successful
    end

    it "should 404 when hash isn't recognised" do
      expect { get :open, params: { delivery_id: 101, hash: "123"} }.to raise_error(ActiveRecord::RecordNotFound)
    end
  ...
  describe "#click" do
    ...
    context "When the correct hash and id are used" do
      context "the delivery_link exists" do
        it "should redirect" do
          get :click, params: { delivery_link_id: 204, url: "http://foo.com", hash: HashId.hash("204-http://foo.com") }
        ...
        it "should log the event" do
          ...
          get :click, params: { delivery_link_id: 204, url: "http://foo.com", hash: HashId.hash("204") }
        end
      end

      context "the delivery_link doesn't exist and url provided" do
        it "should redirect to to the given url" do
          get :click, params: { delivery_link_id: 123, hash: HashId.hash("123-http://bar.com?foo=baz"), url: "http://bar.com?foo=baz" }
      ...
      context "the url has been changed" do
        it "should not redirect to the given url" do
          expect{ get :click, params: { delivery_link_id: 123, hash: HashId.hash("123-http://bar.com?foo=baz"), url: "http://bar.com2?foo=baz" } }.to raise_error(ActiveRecord::RecordNotFound)
        end
      end
    end
    ...
    context "read only mode" do
      ...
      it "should redirect" do
        get :click, params: { delivery_link_id: 204, hash: HashId.hash("204-http://foo.com"), url: "http://foo.com" }
        expect(response).to redirect_to("http://foo.com")
      end
    ...
    it "should 404 when the wrong id is used" do
      expect { get :click, params: { delivery_link_id: 122, hash: "a5ff8760c7cf5763a4008c338d617f71542e362f" } }.to raise_error(ActiveRecord::RecordNotFound)
    end

    it "should 4040 when the wrong hash is used" do
      expect { get :click, params: { delivery_link_id: 204, hash: "123" } }.to raise_error(ActiveRecord::RecordNotFound)
    end
  end
end
