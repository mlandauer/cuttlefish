...
describe ApplicationHelper, type: :helper do
  ...
  describe "#nav_menu_item_show_active" do
    it "should create the simple markup required" do
      expect(helper.nav_menu_item_show_active("Test email", "/foo/bar")).to eq '<li><a href="/foo/bar">Test email</a></li>'
    end

    it "should handle a block argument" do
      expect(helper.nav_menu_item_show_active("/foo/bar") { "Test email" }).to eq '<li><a href="/foo/bar">Test email</a></li>'
    end
    ...
    context "/foo/bar is current page" do
      ...
      it "should be active" do
        expect(helper.nav_menu_item_show_active("Test email", "/foo/bar")).to eq '<li class="active"><a href="/foo/bar">Test email</a></li>'
      end
    end
  end
end
