...
describe DenyList do
  describe "#has_record_of_deny_list_cause?" do
    context "when the caused_by_delivery is missing" do
      let(:deny_list) { create(:deny_list, caused_by_delivery: nil ) }

      it { expect(deny_list.has_record_of_cause?).to be false }
    end

    context "when the caused_by_delivery present but has no subject" do
      let(:deny_list) do
        delivery = create(:delivery, email: create(:email, subject: nil))
        create(:deny_list, caused_by_delivery: delivery)
      end
    ...
    context "when the caused_by_delivery present and has a subject" do
      ...
      let(:deny_list) { create(:deny_list, caused_by_delivery: delivery) }
      ...
      it do
        expect(deny_list.has_record_of_cause?).to be true
      end
    end
  end
end
