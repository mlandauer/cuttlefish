class AddForeignKeyConstraints < ActiveRecord::Migration
  def change
    # Remove any postfix_log_lines that don't belong to a delivery anymore
    non_existing_ids = PostfixLogLine.distinct(:delivery_id).pluck(:delivery_id) - Delivery.pluck(:id)
    PostfixLogLine.where(delivery_id: non_existing_ids).delete_all

    add_foreign_key(:emails, :apps, dependent: :delete)
    add_foreign_key(:deliveries, :emails, dependent: :delete)
    add_foreign_key(:postfix_log_lines, :deliveries, dependent: :delete)
    add_foreign_key(:open_events, :deliveries, dependent: :delete)
    add_foreign_key(:delivery_links, :deliveries, dependent: :delete)
    add_foreign_key(:click_events, :delivery_links, dependent: :delete)
  end
end
